...
# ===================== 1. 数据准备 =====================
# 读取中性化因子数据
neutral_df = pd.read_csv("中性化_消费50品质因子.csv")

# 提取因子列名
factor_columns = ['Inventory_Turnover_neutral','TotalAsset_Turnover_neutral','CurrentAsset_Turnover_neutral','Capital_Fixed_Ratio_neutral']
...
def plot_comparison_revised(ortho_results, neutral_results, metrics_df):
    ...
    factor_names = ['Inventory_Turnover_neutral','TotalAsset_Turnover_neutral','CurrentAsset_Turnover_neutral','Capital_Fixed_Ratio_neutral']
    ...
    try:
        plt.savefig("品质因子处理方法对比.png", dpi=300, bbox_inches='tight')
        print("图像已成功保存至: 品质因子处理方法对比.png")
...
# 保存结果到CSV
try:
    ortho_results.to_csv("品质矩阵分解正交化中性化因子模拟结果.csv", index=False)
    neutral_results.to_csv("品质线性回归中性化因子模拟结果.csv", index=False)
    metrics_df.to_csv("品质因子处理方法对比指标.csv", index=False)
    print("结果已成功保存至CSV文件")
except Exception as e:
    print(f"保存结果时出错: {e}")